...
function init() {
  ...
  for (var i = 0; i < Math.PI * 2; i += dr)
    pointsOrigin.push(scaleAndTranslate(heartPosition(i), 160, 9, 0, 0));
  for (var i = 0; i < Math.PI * 2; i += dr)
    pointsOrigin.push(scaleAndTranslate(heartPosition(i), 120, 6, 0, 0));
  for (var i = 0; i < Math.PI * 2; i += dr)
  ...
  function pulse(kx, ky) {
    for (var i = 0; i < pointsOrigin.length; i++) {
      targetPoints[i] = [
        ...
        ky * pointsOrigin[i][1] + height / 3.6,
      ];
    }
  }
  ...
  function loop() {
    ...
    for (var i = e.length; i--; ) {
      ...
      // drawText() intentionally disabled
